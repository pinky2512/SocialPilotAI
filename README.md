...
- ✅ STORY-035 Process Data Export Requests Efficiently — export is now a queued request (`pending → completed`), processed in a **batch** with the result **stored** so downloads don't recompute; indexed by status. Privacy page has a queued-requests table with per-request download. **Completes R8 → Phase 3 sign-off gate.**
### Phase 4 — Launch Readiness + Go-Live — 🔄 R9 in progress (STORY-036…044)
- ✅ STORY-036 Role-Based Access Control for Content Approval — approving/rejecting held items now requires the new `content:approve` permission (Administrator only). Campaign Managers create and submit, but the approve/reject routes return **403** for them and the denial is audited as `access.denied`; the Approval Queue hides the Approve/Reject buttons for non-approvers. Separation of duties: **AI proposes, Manager submits, Administrator approves.**
